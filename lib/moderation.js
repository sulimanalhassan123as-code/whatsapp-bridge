...
const VISION_MODEL = process.env.GROQ_VISION_MODEL || ''; // Groq removed all llama vision models — empty = skip image scan (fail safe)
...
async function classifyImage(base64Data, mimetype) {
  if (!VISION_MODEL) return { flag: false, category: 'none', reason: '' }; // Groq removed all vision models — skip image scan (fail safe)
  try {
    const raw = await groq.chat([
      {
        role: 'user',
    ...
    ], { model: VISION_MODEL, json: true, maxTokens: 600, temperature: 0 });
    const parsed = JSON.parse(raw);
    return { flag: !!parsed.flag, category: parsed.category || 'none', reason: parsed.reason || '' };
  } catch (e) {
