...
// Simulate human typing delay (0.8s–3.5s) so the bot doesn't reply instantly
// which is a classic bot detection signal on WhatsApp.
async function humanDelay(text) {
  const wpm = 40; // average typing speed
...
const RESPECT_RULE =
  ...
  "IMPORTANT — language: always reply in the SAME language the person just wrote in. If their message is in French, Twi, Hausa, Arabic, or anything other than English, reply fluently and naturally in that same language — never default to English unless they wrote in English.";
...
async function generateWelcomeReply(state) {
  ...
  const toneLine = tone
    ? `Match this specific tone/style for the welcome, exactly as instructed: "${tone}". `
    : '';
  const messages = [
    {
      role: 'system',
...
async function handleIncoming(sock, jid, msg) {
  ...
  // Moderation check runs on every incoming DM, regardless of takeover state.
  let flagged = null;
  if (text) {
  ...
  if (!flagged && hasImage) {
    try {
      ...
      console.error('dm image moderation error:', e.message);
    }
  }
  ...
  if (flagged) {
    ...
    if (newCount <= 1) {
      try {
        await sock.sendMessage(jid, { text: "Please keep this conversation respectful — I'd like to keep things civil. Continued messages like this may lead to being blocked." });
      } catch (e) { /* ignore */ }
      await notifyOwner(`⚠️ Flagged a message from <b>${state.name}</b> (${jid.split('@')[0]}) as ${flagged.category} — sent them a warning.\n\nReason: ${flagged.reason}`);
    } else {
      try {
      ...
      await notifyOwner(`🚫 Blocked <b>${state.name}</b> (${jid.split('@')[0]}) after repeated inappropriate messages.\n\nLast reason: ${flagged.reason}\n\nUse /unblock ${jid.split('@')[0]} on Telegram if this was a mistake.`);
    }
    return;
  }

  if (!text) return; // pure image with nothing flagged — nothing to converse about, stay quiet
