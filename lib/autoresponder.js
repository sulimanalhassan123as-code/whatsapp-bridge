  ...
  const messages = [
    {
      ...
      content:
        ...
        "Reply warmly and naturally in his relaxed, friendly texting voice — " +
  ...
  const messages = [
    {
      ...
      content:
        ...
        "Send a warm, friendly, brief welcome reply in his relaxed, easygoing voice — acknowledge what they said naturally, like a normal person greeting someone new. " +
        "Do NOT reveal you are an AI or bot unless asked directly. Do NOT make commitments, quote prices, or share sensitive info. " +
        "Keep it short and human, 1-3 sentences."
    },
    ...state.history
  ];
  return (await groq.chat(messages, { maxTokens: 250, temperature: 0.7 })).trim();
}
...
async function handleIncoming(sock, jid, msg) {
  ...
  if (contact.always_chat) {
    // flagged as a friend the AI can always chat with — respond immediately, no idle wait
    state.takeoverActive = true;
    try {
