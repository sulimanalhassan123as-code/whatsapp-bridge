...
async function getWelcomeTone() {
  try {
    const resp = await fetch(`${SUPABASE_URL}/rest/v1/wa_config?id=eq.1&select=welcome_tone`, { headers });
    const rows = await resp.json().catch(() => []);
    return Array.isArray(rows) && rows.length ? rows[0].welcome_tone : null;
  } catch (e) {
    console.error('getWelcomeTone error:', e.message);
    return null;
  }
}
...
module.exports = { getWelcomeTemplate, getWelcomeTone, getAntilinkEnabled, setAntilinkEnabled };
